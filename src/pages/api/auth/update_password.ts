...
export async function POST(context: APIContext): Promise<Response> {
	try {
		...
		if (newPassword.length < 8 || newPassword.length > 20) {
			return new Response(JSON.stringify({ error: 'New password must be between 8 and 20 characters' }), {
				status: 400,
				headers: {
					'Content-Type': 'application/json',
				},
		...
		return new Response(JSON.stringify({ error: 'An error occurred while updating the password' }), {
			status: 500,
			headers: {
				'Content-Type': 'application/json',
			},
